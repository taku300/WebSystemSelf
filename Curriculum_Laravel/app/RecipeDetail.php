...
class RecipeDetail extends Model
{
    ...
    public function calNutrients() {
        ...
        ->select('recipes.id','recipes.name', 'recipes.image','recipes.memo', 'recipes.user_id', 'recipe_details.recipe_id')
    ...
    public function calNutrientsSearch($keyword) {
        // \DB::enableQueryLog();
        $recipes = RecipeDetail::join('foods', 'recipe_details.food_id', '=', 'foods.id')
        ->join('recipes', 'recipe_details.recipe_id', '=', 'recipes.id')
        ->select('recipes.id','recipes.name', 'recipes.image','recipes.memo', 'recipes.user_id', 'recipe_details.recipe_id')
        ...
        ->where('recipes.del_flg', '=', 0)
        ->groupBy('recipes.id')
        ->having('recipes.name', 'LIKE', "%{$keyword}%")
        ->get();
        // dd(\DB::getQueryLog());
        return $recipes;
    }
